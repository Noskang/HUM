...
public class EasySplinePath2DEditor : Editor
{
    ...
    void Draw()
    {
        ...
        foreach (int i in anchorsAux)
        {
            ...
            var fmh_869_74_638428556061849983 = Quaternion.identity; Vector2 newPos = Handles.FreeMoveHandle(creator.GetPoint(i), handleSize, new Vector2(), Handles.CircleHandleCap);
        ...
        if (!creator.hideControlPoints)
        {
            foreach (int i in controlsAux)
            {
                ...
                if (!esconder)
                {
                    var fmh_1092_82_638428556061880517 = Quaternion.identity; Vector2 newPos = Handles.FreeMoveHandle(creator.GetPoint(i), handleSize, new Vector2(), Handles.CircleHandleCap);
